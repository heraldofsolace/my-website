...
// Adapted from React Bits' DriftWall (https://reactbits.dev). This one has
// no animation-library dependency upstream — a requestAnimationFrame loop
// writing transforms directly, the right tool for "N columns drifting
// forever at their own speed," which isn't something framer-motion has a
// spring/tween primitive for (same reasoning as this site's other
// vanilla-rAF React Bits ports). Kept as-is.
//
// Real changes from upstream:
//  - `<img>` → next/image's `Image` (`fill`, `unoptimized`) — same reason
//    as every other external-logo image in this codebase.
//  - `object-cover` → `object-contain` plus an extra padded inset around
//    the image: upstream's tiles are full-bleed photography, but this
//    site's items are small square logo marks that would otherwise get
//    cropped/stretched to fill a landscape tile.
//  - Added a name caption, revealed with the same `.is-active` group
//    state upstream already toggles on hover/focus (previously only used
//    to un-dim the tile and clear its overlay) — upstream carries a
//    `title` per item but never actually renders it anywhere.
//  - Tile chrome restyled for the dark theme (bg-bg-soft instead of
//    upstream's near-identical-but-unrelated #0b0b12 hardcoded hex).
//  - Upstream's hand-rolled useState+useEffect for prefers-reduced-motion
//    swapped for this codebase's useMediaQuery (useSyncExternalStore-based,
//    built for the nav sidebar breakpoint) — same job, already on hand.
import {
  useCallback,
  useEffect,
  useLayoutEffect,
...
export default function DriftWall({
  ...
  // useMediaQuery (useSyncExternalStore-based — see its own comment) in
  // place of upstream's hand-rolled useState+useEffect pair for the same
  // "read a matchMedia value reactively" job; this codebase already has
  // it (built for the nav sidebar breakpoint), no reason to duplicate it.
  ...
  const innerClass = cx(
    "pointer-events-none absolute inset-[calc(var(--dw-gap)/2)] block overflow-hidden bg-bg-soft",
  ...
  // Upstream's items carry a `title` but never render it — this site wants
  // the name to appear specifically on hover/focus, so it reuses the same
  // `.is-active` toggle that already un-dims the tile, over a scrim so it
  // stays legible against whatever's in the logo underneath it.
  const captionWrapClass = cx(
    "pointer-events-none absolute inset-x-0 bottom-0 flex items-end justify-center p-3",
    "bg-gradient-to-t from-bg/90 to-transparent opacity-0",
  ...
  const imgClass = cx(
    "block h-full w-full select-none object-contain p-[18%]",
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <span className={innerClass}>
        ...
        {item.title && (
          <span className={captionWrapClass} aria-hidden="true">
            <span className="truncate font-mono text-xs uppercase tracking-widest text-fg">
              {item.title}
            </span>
          </span>
        )}
